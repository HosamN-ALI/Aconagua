...
- Modules communicate via **Events** (using NestJS Event Emitter) or **Direct Service Injection** (only for read operations).
- Write operations across modules MUST use Events to maintain loose coupling.

## Testing Strategy
- Each module MUST have its own test suite.
- Integration tests should mock external module dependencies.
- E2E tests can test the full flow across modules.
